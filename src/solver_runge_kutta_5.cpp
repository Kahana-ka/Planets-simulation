...
std::tuple<double, double, double> rk5::gravity_k1(const rk::Planet_solver& self,const std::vector<rk::Planet_solver>& planets_solver, const double h) {
	double kx = 0;
	double ky = 0;
	double kz = 0;
...
std::tuple<double, double, double> rk5::position_k1(const rk::Planet_solver& self, const double h) {
...
std::tuple<double, double, double> rk5::gravity_k2(const rk::Planet_solver& self,const std::vector<rk::Planet_solver>& planets_solver, const double h) {
...
std::tuple<double, double, double> rk5::position_k2(const rk::Planet_solver& self, const double h) {
...
double rk5::arg_k2(const double param, const double k1) {
	return param + k1 / 4.;
}

//k3

std::tuple<double, double, double> rk5::gravity_k3(const rk::Planet_solver& self,const std::vector<rk::Planet_solver>& planets_solver, const double h) {
...
std::tuple<double, double, double> rk5::position_k3(const rk::Planet_solver& self, const double h) {
...
double rk5::arg_k3(const double param, const double k1, const double k2) {
	return param + 3. * k1 / 32. + 9. / 32. * k2;
}

//k4

std::tuple<double, double, double> rk5::gravity_k4(const rk::Planet_solver& self,const std::vector<rk::Planet_solver>& planets_solver, const double h) {
...
std::tuple<double, double, double> rk5::position_k4(const rk::Planet_solver& self, const double h) {
...
double rk5::arg_k4(const double param, const double k1, const double k2, const double k3) {
	return param + k1*1932./2197. - k2*7200./2197. + k3*7296./2197.;
}

//k5

std::tuple<double, double, double> rk5::gravity_k5(const rk::Planet_solver& self,const std::vector<rk::Planet_solver>& planets_solver, const double h) {
...
std::tuple<double, double, double> rk5::position_k5(const rk::Planet_solver& self, const double h) {
...
double rk5::arg_k5(const double param, const double k1, const double k2, const double k3, const double k4) {
	return param + k1*439./216. - k2*8. + k3*3680./531. - k4*845./4140.;
}

//k6

std::tuple<double, double, double> rk5::gravity_k6(const rk::Planet_solver& self,const std::vector<rk::Planet_solver>& planets_solver, const double h) {
	...
	for (auto& pl : planets_solver) {
		...
		const double x1 = arg_k6(self.planet.pos_x.back(), get<x>(self.k_pos[K1]), get<x>(self.k_pos[K2]), get<x>(self.k_pos[K3]), get<x>(self.k_pos[K4]), get<x>(self.k_pos[K5]));
		const double x2 = arg_k6(pl.planet.pos_x.back(), get<x>(pl.k_pos[K1]), get<x>(pl.k_pos[K2]), get<x>(pl.k_pos[K3]), get<x>(pl.k_pos[K4]), get<x>(pl.k_pos[K5]));

		const double y1 = arg_k6(self.planet.pos_y.back(), get<y>(self.k_pos[K1]), get<y>(self.k_pos[K2]), get<y>(self.k_pos[K3]), get<y>(self.k_pos[K4]), get<y>(self.k_pos[K5]));
		const double y2 = arg_k6(pl.planet.pos_y.back(), get<y>(pl.k_pos[K1]), get<y>(pl.k_pos[K2]), get<y>(pl.k_pos[K3]), get<y>(pl.k_pos[K4]), get<y>(pl.k_pos[K5]));

		const double z1 = arg_k6(self.planet.pos_z.back(), get<z>(self.k_pos[K1]), get<z>(self.k_pos[K2]), get<z>(self.k_pos[K3]), get<z>(self.k_pos[K4]), get<z>(self.k_pos[K5]));
		const double z2 = arg_k6(pl.planet.pos_z.back(), get<z>(pl.k_pos[K1]), get<z>(pl.k_pos[K2]), get<z>(pl.k_pos[K3]), get<z>(pl.k_pos[K4]), get<z>(pl.k_pos[K5]));
...
std::tuple<double, double, double> rk5::position_k6(const rk::Planet_solver& self, const double h) {
	const double kx = arg_k6(self.planet.speed_x.back(), get<x>(self.k_speed[K1]), get<x>(self.k_speed[K2]), get<x>(self.k_speed[K3]), get<x>(self.k_speed[K4]), get<x>(self.k_speed[K5]));
	const double ky = arg_k6(self.planet.speed_y.back(), get<y>(self.k_speed[K1]), get<y>(self.k_speed[K2]), get<y>(self.k_speed[K3]), get<y>(self.k_speed[K4]), get<y>(self.k_speed[K5]));
	const double kz = arg_k6(self.planet.speed_z.back(), get<z>(self.k_speed[K1]), get<z>(self.k_speed[K2]), get<z>(self.k_speed[K3]), get<z>(self.k_speed[K4]), get<z>(self.k_speed[K5]));
	return std::tuple{kx* h, ky* h, kz* h};
}

double rk5::arg_k6(const double param, const double k1, const double k2, const double k3, const double k4, const double k5) {
	return param - k1 * 8. / 27. + k2 * 2. - k3 * 3544. / 2565. + k4 * 1859. / 4104. - k5*11./40.;
}



double rk5::next_point(const double w, const double k1, const double k2, const double k3, const double k4,const double k5, const double k6) {
	return w + 16./135.*k1 + 0.*k2 + 6656./12825.*k3 + 28561./56430.*k4 - 9./50.*k5 +2./55.*k6;
}

void rk5::next_step(std::vector<rk::Planet_solver>& ps,const double h) {
	...
	for (auto& pl : ps) {
		...
		pl.k_speed[K6] = position_k6(pl,h);
	}
	...
	for (auto&[planet, dead, k_pos, k_speed] : ps) {
		...
		planet.pos_x.push_back(next_point(planet.pos_x.back(), get<x>(k_pos[K1]), get<x>(k_pos[K2]), get<x>(k_pos[K3]), get<x>(k_pos[K4]), get<x>(k_pos[K5]), get<x>(k_pos[K6])));
		planet.pos_y.push_back(next_point(planet.pos_y.back(), get<y>(k_pos[K1]), get<y>(k_pos[K2]), get<y>(k_pos[K3]), get<y>(k_pos[K4]), get<y>(k_pos[K5]), get<y>(k_pos[K6])));
		planet.pos_z.push_back(next_point(planet.pos_z.back(), get<z>(k_pos[K1]), get<z>(k_pos[K2]), get<z>(k_pos[K3]), get<z>(k_pos[K4]), get<z>(k_pos[K5]), get<z>(k_pos[K6])));

		planet.speed_x.push_back(next_point(planet.speed_x.back(), get<x>(k_speed[K1]), get<x>(k_speed[K2]), get<x>(k_pos[K3]), get<x>(k_pos[K4]), get<x>(k_pos[K5]), get<x>(k_pos[K6])));
		planet.speed_y.push_back(next_point(planet.speed_y.back(), get<y>(k_speed[K1]), get<y>(k_speed[K2]), get<y>(k_pos[K3]), get<y>(k_pos[K4]), get<y>(k_pos[K5]), get<y>(k_pos[K6])));
		planet.speed_z.push_back(next_point(planet.speed_z.back(), get<z>(k_speed[K1]), get<z>(k_speed[K2]), get<z>(k_pos[K3]), get<z>(k_pos[K4]), get<z>(k_pos[K5]), get<z>(k_pos[K6])));
	}
...
void rk5::RK5_solver::runge_kutta_solver() {
...
rk5::RK5_solver::RK5_solver(const std::vector<phy::Planet>& planets, const double start_time, const double end_time, const double step, const double collision_tollerrance) :
	RK5_solver(planets, start_time, end_time, step, collision_tollerrance, "", true, 100) {
}
